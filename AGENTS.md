...
Present state: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md). Direction: [docs/ROADMAP.md](docs/ROADMAP.md).
...
heart. The probes and replay exist to exercise the real integration.

## How it's built

- **The app owns the pedagogy, not the model** — deterministic `pending → recast → completed` rules
  in the server; the prompt is rebuilt from objective state each turn.
- **Provider-agnostic** — understand (E2), voice (E3), image (E4) run through swappable adapters in
  `server/adapters/`; the swap point is one `.env` line.
- **Scenarios are data** — one JSON per scenario in `server/scenarios/`.
