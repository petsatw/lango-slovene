...
- **Client** (`public/app.js`): `reinforceFromDialogue()` closes the overlay and calls
  `applyChatMode(L2, dialogue.introduces)`. The focus set is carried on the client (like the pinned
  role) and POSTed back on every `/api/converse` turn as `focusLearnables`.
