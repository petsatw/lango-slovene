...
- **Reference sheet** = each bible asset drawn ONCE on a neutral background, labelled. Every flashcard and the scene are then generated by *editing from the sheet* (≤3 refs), so they all inherit the same canonical render.
