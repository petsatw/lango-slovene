...
export interface VoiceProfile {
  id: string;
  description: string;
  /** Short listening noises this voice makes — "Mhm.", "Aha." — pregenerated by `build:backchannels` so
   *  a surface can fire one the instant the learner stops speaking, before any processing could return.
   *  Not dialogue nodes: they belong to the VOICE, not to any one tree. Absent → this voice has none. */
  backchannels?: string[];
}
...
export function loadCatalog(): Catalog {
  ...
  for (const id of Object.keys(profiles)) {
    const bc = profiles[id].backchannels;
    if (bc !== undefined && (!Array.isArray(bc) || bc.some((t: any) => typeof t !== "string" || !t)))
      fail(`voices.json/${id}: "backchannels" must be an array of non-empty strings`);
    voiceProfiles[id] = {
      id,
      description: asString(profiles[id], "description", `voices.json/${id}`),
      ...(bc?.length ? { backchannels: bc as string[] } : {}),
    };
  }
